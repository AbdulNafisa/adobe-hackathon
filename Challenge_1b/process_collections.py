...
class PDFProcessor:
    ...
    def _is_section_header(self, line: str) -> bool:
        ...
        if re.match(r'^[A-Z][a-z]+(?:\s+[a-z]+)*$', line) and len(line.split()) <= 5:
            return True
...
class RelevanceScorer:
    """Scores sections based on relevance to persona and job."""
    
    def __init__(self):
        # Enhanced keywords for different personas and jobs
        self.persona_keywords = {
            ...
            'hr professional': ['form', 'fillable', 'onboarding', 'compliance', 'employee', 'hr', 
                              ...
                              'edit', 'export', 'share', 'request', 'send', 'signatures'],
    ...
    def score_section(self, section: Dict[str, Any], persona: str, job: str) -> float:
        ...
        score = (keyword_matches * 2) + (title_relevance * 5) + min(content_length / 50, 3)
        
        # Bonus for exact job-related terms
        if any(term in title.lower() for term in ['form', 'fillable', 'falafel', 'ratatouille', 'coastal']):
            score += 10
...
class CollectionProcessor:
    """Main processor for handling collections."""
...
def main():
    """Main entry point."""
    if len(sys.argv) < 2:
        print("Usage: python process_collections.py <collection_path>")
        print("Example: python process_collections.py Collection_1")
        sys.exit(1)
